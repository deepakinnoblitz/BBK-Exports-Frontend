...
function getLineShortCode(name?: string | null): string {
  if (!name) return '-';
...
export function LineRosterMonthlyView({
  canEdit = true,
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (selectedLineFilter !== 'all') count += 1;
    return count;
  }, [selectedDept, selectedEmployees, selectedLineFilter]);

  const canReset = selectedDept !== 'all' || selectedEmployees.length > 0 || selectedLineFilter !== 'all' || !!searchEmployee;
  ...
  const handleDrawerFilters = (update: any) => {
    ...
    if (update.line_order !== undefined || update.line !== undefined) {
      setSelectedLineFilter(update.line_order || update.line || 'all');
    }
  };

  // Color map for lines
  const lineColorMap: Record<string, { bg: string; color: string; border: string }> = {};
  data?.lines?.forEach((l, idx) => {
    lineColorMap[l.name] = LINE_PALETTES[idx % LINE_PALETTES.length];
  });

  const filteredEmployees = (data?.employees || []).filter((emp) => {
    if (selectedLineFilter && selectedLineFilter !== 'all') {
      const hasLine = Object.values(emp.lines || {}).some(
        (cell: any) => cell?.line_order === selectedLineFilter || cell?.line_name === selectedLineFilter
      );
    ...
    <Stack spacing={2.5}>
          ...
          <Stack direction="row" gap={1.5} alignItems="center" flexWrap="wrap">
            ...
            <Autocomplete
              ...
              value={employees.filter((opt) =>
                selectedEmployees.some((se) => (typeof se === 'string' ? se === opt.name : se?.name === opt.name))
              )}
      ...
      <Card sx={{ p: 2.5 }}>
        ...
        <Stack direction="row" spacing={2} sx={{ mb: 2.5, flexWrap: 'wrap', gap: 1.5 }} alignItems="center">
          ...
          {data?.lines?.map((l) => {
            const pal = lineColorMap[l.name] || { bg: '#e2e8f0', color: '#475569', border: '#cbd5e1' };
            const code = getLineShortCode(l.line_name || l.name);
            return (
              ...
              {loading ? (
                ...
                filteredEmployees.map((emp, empIndex) => (
                  <TableRow key={emp.employee} hover sx={{ '& td': { py: 1.2 } }}>
                    ...
                    {/* Day Line Cells */}
                    {data?.days?.map((d) => {
                      const isFullHoliday = d.is_holiday && !filteredEmployees.some((e) => e.lines?.[d.date]?.source === 'ROSTER');
                      ...
                      } else if (cell?.line_order || cell?.line_name) {
                        const lineKey = cell.line_order || cell.line_name;
                        const pal = lineColorMap[lineKey] || { bg: '#e0f2fe', color: '#0284c7', border: '#bae6fd' };
                        chipCode = getLineShortCode(cell.line_name || cell.line_order);
                        chipStyle = pal;
                      }

                      const tooltipText = (isHoliday && cell?.source !== 'ROSTER')
                        ? `Holiday: ${d.holiday_name || 'Public Holiday'}\n${d.date}`
                        : (cell?.line_order || cell?.line_name)
                        ? `${cell.line_name || cell.line_order}\n${d.date}\nSource: ${cell.source || 'ROSTER'}`
                        : `Unassigned\n${d.date}`;
