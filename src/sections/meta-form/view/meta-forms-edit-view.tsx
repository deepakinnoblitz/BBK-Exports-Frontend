        ...
        if (id) {
            ...
            getMetaForm(id)
                .then((data) => {
                    ...
                    setAdSetName(data.ad_set_name || '');
                    setAdId(data.ad_id || '');
                    setAdName(data.ad_name || '');
                    ...
                    if (data.field_mappings && data.field_mappings.length > 0) {
                        ...
                        // Fallback auto populate required fields if empty
                        const defaultMetaKeys: Record<string, string> = {
                            lead_name: 'full_name',
                            email: 'email',
                            phone_number: 'phone_number',
                        };
                        const autoMappings = (mandatoryFields || []).map((field: any) => ({
                ...
                .catch(() => enqueueSnackbar('Failed to load Meta Form data.', { variant: 'error' }))
        ...
        try {
            await updateMetaForm(id!, {
                ...
                field_mappings: processedMappings.map(m => ({
                    ...
                    default_value: (m.default_value || '').trim(),
                    ...
                    <Stack spacing={3}>
                        ...
                        <Box sx={{ display: 'grid', gap: 3, gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' } }}>
                            <TextField
                                ...
                                helperText={errors.formName ? 'Form Name is required' : ''}
                            />
                            <TextField
                                fullWidth
                                label="Form ID"
                                ...
                                error={errors.formId}
                                helperText={errors.formId ? 'Form ID is required' : ''}
                            />
                        </Box>
