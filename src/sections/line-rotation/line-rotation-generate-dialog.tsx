...
import {
  getLineRotationDoc,
  generateLineRotationAssignments,
...
type Props = {
  open: boolean;
  onClose: () => void;
  rotations?: LineRotation[];
  selectedRotationName?: string | null;
  onSuccess: (message: string) => void;
  onError: (error: string) => void;
};

export function LineRotationGenerateDialog({
  open,
  onClose,
  rotations,
  ...
  const [fetchedRotations, setFetchedRotations] = useState<LineRotation[]>([]);
  const availableRotations = rotations && rotations.length > 0 ? rotations : fetchedRotations;

  const [selectedName, setSelectedName] = useState<string>('');
  const [rotationDetail, setRotationDetail] = useState<LineRotation | null>(null);
  ...
  useEffect(() => {
    if (selectedName) {
      ...
      getLineRotationDoc(selectedName)
        .then((doc) => {
  ...
  const loadMasters = async () => {
    try {
      if (!rotations || rotations.length === 0) {
        const rotRes = await getDoctypeList('Line Rotation', ['name', 'rotation_name', 'frequency', 'status', 'department']);
  ...
  const handleGenerate = async () => {
    if (!selectedName) {
      onError('Please select a Line Rotation.');
      return;
    }
    ...
    try {
      setSubmitting(true);
      const res: any = await generateLineRotationAssignments({
      ...
      onSuccess(res?.message || `Line Roster generated for ${selectedEmployeeIds.length} employees.`);
      onClose();
    } catch (err: any) {
      console.error(err);
      onError(err?.message || 'Failed to generate line roster from rotation.');
    } finally {
      setSubmitting(false);
    }
  };
          ...
          <Stack spacing={3} sx={{ mt: 1 }}>
                    ...
                    <Stack direction="row" alignItems="center" spacing={1.25}>
                      ...
                      <div>
                        ...
                        <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 600, display: 'block', fontSize: '0.75rem' }}>
                          {rotationDetail.sequences?.length || 0} sequential line order{(rotationDetail.sequences?.length || 0) === 1 ? '' : 's'} in continuous rotation
                        </Typography>
                      </div>
                    </Stack>
                  ...
                  <div>
                    ...
                    <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 500 }}>
                      {selectedEmployeeIds.length === 0
                        ...
                        : 'Ready to receive the recurring line rotation schedule.'}
                    </Typography>
                  </div>
                </Stack>
            ...
            {overrideConflicts && (
              <Alert severity="warning" sx={{ borderRadius: 1.5 }}>
                Any active line roster assignments on overlapping dates for selected employees will be <b>cancelled and replaced</b> with rotation line orders.
              </Alert>
            )}
          </Stack>
        </DialogContent>
