import type { SelectorLineEmployeeItem } from 'src/api/line-rotation';
...
import {
  fetchSelectorLineEmployees,
  fetchSelectorLineEmployeeIds,
...
export function EmployeeSelectorDialog({
  ...
  const [employees, setEmployees] = useState<SelectorLineEmployeeItem[]>([]);
  ...
  const loadEmployees = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetchSelectorLineEmployees({
  ...
  const handleSelectAllFiltered = async () => {
    setBulkLoading(true);
    try {
      const allIds = await fetchSelectorLineEmployeeIds({
        department: selectedDept?.name || 'all',
        shift: selectedShift?.name || 'all',
        line_order: selectedLine?.name || 'all',
        search,
              ...
              <Table size="small" stickyHeader>
                <TableHead>
                  ...
                  >
                    ...
                    <TableCell sx={{ minWidth: 130 }}>Line</TableCell>
