...
export function PurchaseEditView() {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                const purchaseContacts = vendors.filter((contact: any) => contact.customer_type === 'Purchase');
                ...
                if (purchaseData) {
                    ...
                    setBillDate(purchaseData.bill_date || new Date().toISOString().split('T')[0]);
                    setPaymentType(purchaseData.payment_type || '');
                    setPaymentTerms(purchaseData.payment_terms || '');
                    setDueDate(purchaseData.due_date || '');
                    setDescription(purchaseData.description || '');
                    setDiscountType(purchaseData.overall_discount_type || 'Flat');
                    setDiscountValue(purchaseData.overall_discount || 0);

                    if (purchaseData.attach) {
                        setAttachments([{ name: 'Attachment', url: purchaseData.attach }]);
                    }

                    if (purchaseData.table_qecz && purchaseData.table_qecz.length > 0) {
                        setItems(purchaseData.table_qecz.map((item: any) => ({
                            name: item.name,
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Upload files if any are new local Files
            let attachmentUrl = '';
            if (attachments.length > 0 && attachments[0] instanceof File) {
                setUploading(true);
                try {
                    const uploaded = await uploadFile(attachments[0], 'Purchase', id, 'attachments');
    ...
    const handlePrint = () => {
        if (id) {
            window.open(getPurchasePrintUrl(id), '_blank');
        }
    };
