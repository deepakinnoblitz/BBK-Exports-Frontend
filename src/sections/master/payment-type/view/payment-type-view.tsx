...
import { usePaymentTypes } from 'src/hooks/use-masters';

import { DashboardContent } from 'src/layouts/dashboard';
import { deletePaymentTypeCustom, PaymentType } from 'src/api/masters';
...
export function PaymentTypesView() {
  ...
  const { data, total, loading, refetch } = usePaymentTypes(
    page + 1,
    rowsPerPage,
    filterName,
    orderBy,
  ...
  const handleEditRow = (row: PaymentType) => {
    setSelectedType(row);
    setOpenForm(true);
  };
  ...
  const handleConfirmDelete = async () => {
    if (confirmDelete.id) {
      try {
        await deletePaymentTypeCustom(confirmDelete.id);
        setSnackbar({ open: true, message: 'Payment Type deleted successfully', severity: 'success' });
        refetch();
      } catch (error: any) {
    ...
    <DashboardContent maxWidth={false} sx={{ mt: 2 }}>
      ...
      <PaymentTypeDialog
        ...
        onSuccess={async () => {
          ...
          setSnackbar({
            open: true,
            message: `Payment Type ${selectedType ? 'updated' : 'created'} successfully`,
            severity: 'success',
          });
          await refetch({
            page: page + 1,
