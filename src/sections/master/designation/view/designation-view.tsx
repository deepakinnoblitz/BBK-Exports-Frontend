...
import { useDesignations } from 'src/hooks/use-masters';

import { deleteDesignation } from 'src/api/masters';
...
export function DesignationView() {
  ...
  const { data, total, loading, refetch } = useDesignations(
    page + 1,
    rowsPerPage,
    filterName,
    orderBy,
    ...
    <DashboardContent maxWidth={false} sx={{mt: 2}}>
      ...
      <DesignationDialog
        ...
        onSuccess={() => {
          setSnackbar({
            open: true,
            message: `Designation ${selectedId ? 'updated' : 'created'} successfully`,
            severity: 'success',
          });
          refetch();
        }}
