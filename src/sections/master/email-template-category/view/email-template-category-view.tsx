...
import { useCrmEmailTemplateCategories } from 'src/hooks/use-masters';
...
import { EmailTemplateCategoryDialog } from '../email-template-category-dialog';
import { EmailTemplateCategoryTableRow } from '../email-template-category-table-row';
...
export function EmailTemplateCategoryView() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<CrmEmailTemplateCategory | null>(null);
  ...
  const { data, total, loading, refetch } = useCrmEmailTemplateCategories(
    page + 1,
    rowsPerPage,
    filterName,
    orderBy,
  ...
  const handleEditRow = (row: CrmEmailTemplateCategory) => {
    ...
    <DashboardContent maxWidth={false} sx={{ mt: 2 }}>
      ...
      <Card>
        <LeadTableToolbar
          ...
          searchPlaceholder="Search Email Template Category..."
      ...
      <EmailTemplateCategoryDialog
        ...
        onSuccess={async () => {
          ...
          setSnackbar({
            open: true,
            message: `Email Template Category ${selectedCategory ? 'updated' : 'created'} successfully`,
            severity: 'success',
          });
          await refetch({
            page: page + 1,
