...
import { useCallStatuses } from 'src/hooks/use-masters';

import { DashboardContent } from 'src/layouts/dashboard';
import { type CallStatus, deleteCallStatus } from 'src/api/masters';
...
export function CallStatusView() {
  ...
  const [selectedCallStatus, setSelectedCallStatus] = useState<CallStatus | null>(null);
  ...
  const { data, total, loading, refetch } = useCallStatuses(
    page + 1,
    rowsPerPage,
    filterName,
    orderBy,
  ...
  const handleEditRow = (row: CallStatus) => {
    setSelectedCallStatus(row);
    setOpenForm(true);
  };
  ...
  const handleConfirmDelete = async () => {
    if (confirmDelete.id) {
      try {
        await deleteCallStatus(confirmDelete.id);
        setSnackbar({ open: true, message: 'Call Status deleted successfully', severity: 'success' });
        refetch();
      } catch (error: any) {
        setSnackbar({ open: true, message: error.message || 'Failed to delete Call Status', severity: 'error' });
      } finally {
    ...
    <DashboardContent maxWidth={false} sx={{ mt: 2 }}>
      ...
      <Card>
        <LeadTableToolbar
          ...
          searchPlaceholder="Search Call Status..."
      ...
      <CallStatusDialog
        ...
        currentCallStatus={selectedCallStatus}
        onSuccess={async () => {
          setOpenForm(false);
          setSelectedCallStatus(null);
          setSnackbar({
            open: true,
            message: `Call Status ${selectedCallStatus ? 'updated' : 'created'} successfully`,
            severity: 'success',
          });
          await refetch({
            page: page + 1,
      ...
      <ConfirmDialog
        ...
        content="Are you sure you want to delete this Call Status?"
        action={
