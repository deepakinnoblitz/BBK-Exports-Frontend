...
import { useShifts } from 'src/hooks/use-masters';

import { deleteShift } from 'src/api/masters';
...
export function ShiftView() {
  const { user } = useAuth();
  const actionPerms = user?.permissions?.actions?.shift;
  ...
  const { data, total, loading, refetch } = useShifts(
    page + 1,
    rowsPerPage,
    filterName,
    orderBy,
  ...
  const handleConfirmDelete = async () => {
    if (confirmDelete.id) {
      try {
        ...
        setSnackbar({ open: true, message: error.message || 'Failed to delete shift', severity: 'error' });
      } finally {
  ...
  return (
    <DashboardContent maxWidth={false} sx={{ mt: 2 }}>
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 5 }}>
        <Typography variant="h4">Shift List</Typography>
        {canCreate && (
          <Button
            variant="contained"
      ...
      <ShiftDialog
        ...
        onSuccess={() => {
          setSnackbar({
            open: true,
            message: `Shift ${selectedId ? 'updated' : 'created'} successfully`,
            severity: 'success',
          });
          refetch();
        }}
