...
import { useLineOrders } from 'src/hooks/use-masters';

import { deleteLineOrder } from 'src/api/masters';
...
import { LineOrderDialog } from '../line-order-dialog';
import { TableNoData } from '../../../lead/table-no-data';
import { LineOrderTableRow } from '../line-order-table-row';
...
export function LineOrderView() {
  const { user } = useAuth();
  const actionPerms = user?.permissions?.actions?.line_order;
  ...
  const { data, total, loading, refetch } = useLineOrders(
    page + 1,
    rowsPerPage,
    filterName,
    orderBy,
