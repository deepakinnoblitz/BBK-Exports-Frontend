...
import { CompanyBankAccountDialog } from '../company-bank-account-dialog';
import { CompanyBankAccountTableRow } from '../company-bank-account-table-row';
import { CompanyBankAccountDetailsDialog } from '../company-bank-account-details-dialog';
...
export function CompanyBankAccountView() {
  ...
  const { data, total, loading, refetch } = useCompanyBankAccounts(
    page + 1,
    rowsPerPage,
    filterName,
    orderBy,
  ...
  const handleEditRow = (row: CompanyBankAccount) => {
  ...
  const handleConfirmDelete = async () => {
    if (confirmDelete.id) {
      try {
        await deleteCompanyBankAccount(confirmDelete.id);
        setSnackbar({ open: true, message: 'Company Bank Account deleted successfully', severity: 'success' });
        refetch();
      } catch (error: any) {
    ...
    <DashboardContent maxWidth={false} sx={{ mt: 2 }}>
      ...
      <Card>
        <LeadTableToolbar
          ...
          searchPlaceholder="Search Company Bank Account..."
        ...
        <TableContainer sx={{ position: 'relative', overflow: 'unset' }}>
          <Scrollbar>
            <Table size="medium" sx={{ minWidth: 960, borderCollapse: 'collapse' }}>
              <LeadTableHead
                order={order}
                orderBy={orderBy}
              ...
              <TableBody>
                {loading ? (
                  ...
                  <>
                    ...
                    {empty && (
                      <MasterEmptyState
                        masterName="Company Bank Account"
                        colSpan={7}
                      />
                    )}
      ...
      <CompanyBankAccountDialog
        ...
        onSuccess={async () => {
          ...
          setSnackbar({
            open: true,
            message: `Company Bank Account ${selectedId ? 'updated' : 'created'} successfully`,
            severity: 'success',
          });
          await refetch({
            page: page + 1,
      ...
      <CompanyBankAccountDetailsDialog
        open={openDetails}
        onClose={() => {
      ...
      <ConfirmDialog
        ...
        content="Are you sure you want to delete this company bank account?"
        action={
