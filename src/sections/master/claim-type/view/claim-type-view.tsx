...
import { useClaimTypes } from 'src/hooks/use-masters';

import { deleteClaimType } from 'src/api/masters';
...
import { ClaimTypeTableRow } from '../claim-type-table-row';
...
export function ClaimTypeView() {
  const { user } = useAuth();
  const actionPerms = user?.permissions?.actions?.claim_type;
  ...
  const { data, total, loading, refetch } = useClaimTypes(
    page + 1,
    rowsPerPage,
    filterName,
    orderBy,
  ...
  return (
    <DashboardContent maxWidth={false} sx={{mt: 2}}>
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 5 }}>
        <Typography variant="h4">Claim Type List</Typography>
        {canCreate && (
          <Button
            variant="contained"
      ...
      <ClaimTypeDialog
        ...
        onSuccess={() => {
          setSnackbar({
            open: true,
            message: `Claim Type ${selectedId ? 'updated' : 'created'} successfully`,
            severity: 'success',
          });
          refetch();
        }}
      ...
      <ConfirmDialog
        ...
        title="Delete"
        content="Are you sure you want to delete this claim type?"
        action={
