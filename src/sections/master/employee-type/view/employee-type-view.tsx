...
import { useEmployeeTypes } from 'src/hooks/use-masters';

import { deleteEmployeeType } from 'src/api/masters';
...
import { EmployeeTypeDialog } from '../employee-type-dialog';
import { LeadTableHead } from '../../../lead/lead-table-head';
import { TableEmptyRows } from '../../../lead/table-empty-rows';
import { EmployeeTypeTableRow } from '../employee-type-table-row';
...
export function EmployeeTypeView() {
  const { user } = useAuth();
  const actionPerms = user?.permissions?.actions?.employee_type || user?.permissions?.actions?.master_employee_type;
  ...
  const { data, total, loading, refetch } = useEmployeeTypes(
    page + 1,
    rowsPerPage,
    filterName,
    orderBy,
  ...
  const handleConfirmDelete = async () => {
    if (confirmDelete.id) {
      try {
        await deleteEmployeeType(confirmDelete.id);
        setSnackbar({ open: true, message: 'Employee Type deleted successfully', severity: 'success' });
        refetch();
      } catch (error: any) {
        setSnackbar({ open: true, message: error.message || 'Failed to delete employee type', severity: 'error' });
      } finally {
  ...
  return (
    <DashboardContent maxWidth={false} sx={{ mt: 2 }}>
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 5 }}>
        <Typography variant="h4">Employee Type List</Typography>
        {canCreate && (
          <Button
            variant="contained"
      ...
      <EmployeeTypeDialog
        ...
        onSuccess={() => {
          setSnackbar({
            open: true,
            message: `Employee Type ${selectedId ? 'updated' : 'created'} successfully`,
            severity: 'success',
          });
          refetch();
        }}
      ...
      <ConfirmDialog
        ...
        title="Delete"
        content="Are you sure you want to delete this employee type?"
        action={
